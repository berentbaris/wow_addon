...
**Current status:** Milestone 4 complete — all three tasks done (4.1 professions, 4.2 talents, 4.3 self-found). `SelfFoundCheck.lua` detects the Self-Found buff via spell-ID scan + name-based fallback (locale-safe), checks "Self-made" challenge items against curated profession-crafted ID lists (white/grey quality always passes, higher quality must appear on the crafted list), and checks "Self-made guns" specifically against an Engineering-gun list. Results stored in `HCE_CharDB.selfFoundResults`. RequirementsPanel summary row now shows self-found buff ✓/✗/? indicator with hover tooltip. CHALLENGES section shows ✓/✗/? for Self-made and Self-made guns rows, with combined tooltip showing challenge description + self-made check detail. Chat warnings fire once per session. `/hce selffound` (and `/hce selfmade`, `/hce sf`) prints full status with per-item breakdown. Warning state resets on character pick/reset. All eleven Lua files parse cleanly.
...
**Task 5.1: Challenge rule engine** — each challenge type (Homebound, Renegade, Partisan, Drifter, Exotic, etc.) gets its own checker module. Structure: a table of challenge definitions with check functions that run on relevant events.
