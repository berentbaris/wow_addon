...
1. Download or clone this folder so that it's named `ClassicClassesEnhanced`
2. Place it in your WoW Classic addons directory:
   ```
   World of Warcraft/_classic_/Interface/AddOns/ClassicClassesEnhanced/
   ```
3. Make sure the folder contains `ClassicClassesEnhanced.toc` at the top level (not nested inside another subfolder)
...
- `ClassicClassesEnhanced.lua` — main addon file, event handling, slash commands
