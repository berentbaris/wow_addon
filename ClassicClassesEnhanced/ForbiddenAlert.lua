----------------------------------------------------------------------
-- ClassicClassesEnhanced — Forbidden-item Alerts
--
...
function Forbid.Fire(ruleDesc, detail, playSound, flashEdge)
    if CCE_GlobalDB and CCE_GlobalDB.forbiddenAlertsEnabled == false then
        return
    end
    ...
    if flashEdge ~= false and (not CCE.EdgeFlashEnabled or CCE.EdgeFlashEnabled()) then
        flashEdges()
    end
end
...
function Forbid.FireBatch(entries)
    ...
    if #entries > 3 then
        ...
        t:Show()
        animateToast(t)
        if not CCE.AlertSoundEnabled or CCE.AlertSoundEnabled() then
            PlaySound(ERROR_SOUND, "Master")
        end
        if not CCE.EdgeFlashEnabled or CCE.EdgeFlashEnabled() then
            flashEdges()
        end
        return
    end
...
----------------------------------------------------------------------
-- Test helper — fire a sample forbidden alert for /cce testforbidden
----------------------------------------------------------------------

function CCE.TestForbiddenAlert()
