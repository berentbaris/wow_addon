----------------------------------------------------------------------
-- HardcoreClassesEnhanced — Event-Based Challenge Tracking
--
...
--
-- Completion is permanent (saved in HCE_CharDB.eventChallenges).
...
local function getDB()
    ...
    if not HCE_CharDB.eventChallenges then
        HCE_CharDB.eventChallenges = {}
    end
    return HCE_CharDB.eventChallenges
end
...
local function CreateRitualFrame()
    ...
    btn:SetScript("OnClick", function()
        ...
        print("|cff9933cc[HCE]|r |cffffcc00Voodoo Ritual complete!|r The spirits of Jintha'Alor acknowledge your dark dance.")
        f:Hide()
...
local function OnCombatLogEvent()
    ...
    if sub == "SPELL_CAST_SUCCESS" or sub == "SPELL_AURA_APPLIED" then
        if sourceName == UnitName("player") and destName == TARGET_CLUNK then
            if REMOTE_SPELLS[spellName] or spellID == REMOTE_SPELL_ID then
                ...
                print("|cffe6b422[HCE]|r |cff00ff00Clunk has been mind-controlled!|r Now defeat Trade Master Kovic.")
            end
        end
    end
    ...
    if sub == "UNIT_DIED" and destName == TARGET_KOVIC then
        ...
        if controlled then
            ...
            print("|cffe6b422[HCE]|r |cffffcc00Gnomish Justice complete!|r Trade Master Kovic has been dealt with.")
            if HCE.ChallengeCheck and HCE.ChallengeCheck.CheckAndWarn then
                C_Timer.After(0.5, HCE.ChallengeCheck.CheckAndWarn)
            end
        end
...
local function CreateRedemptionFrame()
    ...
    btn:SetScript("OnClick", function()
        ...
        if not where then
            print("|cffcc3333[HCE]|r Scarlet Tabard not found.")
            return
        end
        ...
        DoEmote("KNEEL")
        print("|cffcc3333[HCE]|r |cffffcc00Scarlet Redemption complete!|r")
        print("|cffcc3333[HCE]|r You have renounced the Scarlet Crusade at Light's Hope Chapel.")
        f:Hide()
        if HCE.ChallengeCheck and HCE.ChallengeCheck.CheckAndWarn then
            C_Timer.After(0.5, HCE.ChallengeCheck.CheckAndWarn)
        end
...
local function CreatePlagueFrame()
    ...
    btn:SetScript("OnClick", function()
        ...
        if not bag then
            print("|cffe6b422[HCE]|r Nightglow Concoction not found in bags.")
            return
        end
        ...
        DoEmote("CACKLE")
        print("|cff44ff44[HCE]|r |cffffcc00The New Plague has been deployed in Southshore!|r")
        print("|cff44ff44[HCE]|r The Royal Apothecary Society's work is complete.")
        f:Hide()
        if HCE.ChallengeCheck and HCE.ChallengeCheck.CheckAndWarn then
            C_Timer.After(0.5, HCE.ChallengeCheck.CheckAndWarn)
        end
