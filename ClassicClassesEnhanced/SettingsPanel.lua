----------------------------------------------------------------------
-- ClassicClassesEnhanced — Settings Panel
--
...
-- button, and panel behaviour.  Accessible via /cce settings.
--
...
local function charDB()
    ...
    CCE_CharDB.charSettings = CCE_CharDB.charSettings or {}
    local key = CCE_CharDB.selectedCharacter
    CCE_CharDB.charSettings[key] = CCE_CharDB.charSettings[key] or {}
    return CCE_CharDB.charSettings[key]
end
...
local function BuildFrame()
    ...
    y = MakeCheckbox(frame, y,
        ...
        function(v)
            db().alertsEnabled = v
            if not v and CCE.Alert then CCE.Alert.DismissAll() end
        end
    )
    ...
    y = MakeCheckbox(frame, y,
        ...
        function(v)
            ...
            if not v and CCE.ForbiddenAlert then CCE.ForbiddenAlert.DismissAll() end
        end
    )

    y = MakeCheckbox(frame, y,
        "Chat warnings",
        "Print gold [CCE] messages in chat when requirements change status (profession behind, wrong talent spec, etc.).",
    ...
    y = MakeCheckbox(frame, y,
        ...
        function(v)
            ...
            if v then
                if CCE.ShowMinimapButton then CCE.ShowMinimapButton() end
            else
                if CCE.HideMinimapButton then CCE.HideMinimapButton() end
            end
        end
    )
    ...
    y = MakeCheckbox(frame, y,
        ...
        function(v)
            ...
            if CCE.SetPanelLocked then CCE.SetPanelLocked(v) end
        end
    )
    ...
    y = MakeCheckbox(frame, y,
        ...
        function(v)
            db().doubtEnabled = v
            if CCE.DoubtSystem then CCE.DoubtSystem.UpdateBar() end
        end
    )
    ...
    resetBtn:SetScript("OnClick", function()
        if SlashCmdList and SlashCmdList["CCE"] then
            SlashCmdList["CCE"]("reset")
        end
        Settings.Refresh()
    end)
    ...
    changeBtn:SetScript("OnClick", function()
        if CCE.ShowSelectionUI then CCE.ShowSelectionUI() end
    end)
    ...
    donateTip:SetText("Type |cffffd100/cce donate|r to copy the link.")
    y = y - 24
    ...
    ver:SetText("Classic Classes Enhanced v" .. (CCE.version or "?"))
...
function CCE.ChatWarningsEnabled()
...
function CCE.AlertSoundEnabled()
...
function CCE.SelfFoundEnabled()
    -- selfFoundChoice is saved at CCE_CharDB top level by CommitSelection
