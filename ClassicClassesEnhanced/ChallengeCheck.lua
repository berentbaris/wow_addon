...
R("Spirit of Aviana", function()
    if not CCE.BehavioralCheck or not CCE.BehavioralCheck.CheckSpellRestriction then
        local _, classToken = UnitClass("player")
        if classToken ~= "DRUID" then
            return PASS, "Not a druid — Spirit of Aviana rule not applicable"
        end
        return UNCHECKED, "Behavioral tracking module not loaded"
    end
    return CCE.BehavioralCheck.CheckSpellRestriction("Spirit of Aviana")
end)
