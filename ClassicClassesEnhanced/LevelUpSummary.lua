----------------------------------------------------------------------
-- ClassicClassesEnhanced — Level-Up Summary
--
...
function Summary.Show(newLevel, items)
    ...
    if CCE.Progress and CCE.Progress.Collect then
        local data = CCE.Progress.Collect()
        pct = CCE.Progress.Percentage(data.counts)
    end
