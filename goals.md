...
**Current status:** Task 5.1 complete — challenge rule engine built. `ChallengeCheck.lua` provides a central registry of checker functions for all 19 challenge types. Quality-based challenges (White Knight, Exotic, Footman, Grunt) scan all gear slots and flag items by rarity. Armor-type challenges (Cloth/leather, Mail/plate) verify armor subclass on body slots. No professions checks via `IsSpellKnown` spell-ID scan. Imp/No demon challenges use `UnitCreatureFamily`/`UnitCreatureType` with locale-aware fallbacks. Item-source challenges (Renegade, Partisan, Off-the-shelf) are wired to curated item-ID lists ready for Milestone 7. Self-made/Self-made guns delegate to SelfFoundCheck. Zone-based (Homebound) and behavioral (Drifter, Ephemeral) return UNCHECKED with clear "planned in task 5.x" messages. Results stored in `HCE_CharDB.challengeResults`. RequirementsPanel CHALLENGES section now shows ✓/✗/? indicators with hover tooltips for ALL challenge types. Chat warnings fire on new violations, forbidden-alert toasts on failures. `/hce challenges` prints full status. Warning state resets on pick/reset. Panel listens to UNIT_PET for pet-related challenges. All twelve Lua files parse cleanly.
...
**Task 5.2: Zone-based challenges** — Homebound (continent restriction) using C_Map.GetBestMapForUnit + C_Map.GetMapInfo to detect continent. Anti-undead/Pro-nature/Anti-demon (must visit specific zones) using ZONE_CHANGED events.
