----------------------------------------------------------------------
-- ClassicClassesEnhanced  —  Addon Communication  (v4)
--
...
function Comm.ToggleDebug()
    ...
    CCE.Print(debugMode and "|cff00ff00AddonComm debug ON|r" or "|cffff6060AddonComm debug OFF|r")
end

local function dbg(msg)
    if debugMode and CCE.Print then
        CCE.Print("|cff888888[dbg] " .. msg .. "|r")
    end
end
...
local function getMyClassName()
    ...
    local full = CCE.GetCharDisplayName and CCE.GetCharDisplayName(char) or char.name
    return COMM_SHORT_NAMES[full] or full
end

local function getMyRank()
    if not CCE.Progress or not CCE.Progress.Collect
       or not CCE.Progress.Percentage or not CCE.Progress.GetRank then
        return "Initiate"
    end
    local s = CCE.Progress.Collect()
    if not s or not s.counts then return "Initiate" end
    return CCE.Progress.GetRank(CCE.Progress.Percentage(s.counts))
end
...
local function queueHello()
    ...
    local selKey = CCE_CharDB and CCE_CharDB.selectedCharacter
    local selChar = selKey and CCE.GetCharacter and CCE.GetCharacter(selKey)
    if selChar then
...
local function acquireScanRow(index)
    ...
    iBtn:SetScript("OnClick", function()
        ...
        if CCE.Print then
            CCE.Print("Invited |cffffffff" .. name .. "|r to group.")
        end
    end)
...
function Comm.PrintStatus()
    ...
    CCE.Print("|cffffd100HCE AddonComm Status:|r")
    CCE.Print("  Channel: " .. CHANNEL_NAME .. " #" .. tostring(id))
    CCE.Print("  My class: " .. tostring(getMyClassName() or "NONE"))
    CCE.Print("  My rank: " .. tostring(getMyRank()))
    CCE.Print("  Debug: " .. (debugMode and "ON" or "OFF"))
    CCE.Print("  Queue: " .. #sendQueue .. " pending")
    local count = 0
    local now = GetTime()
    for _, e in pairs(playerCache) do
        if (now - e.time) < CACHE_TTL then count = count + 1 end
    end
    CCE.Print("  Cached players: " .. count)
    local cl = { GetChannelList() }
    if #cl > 0 then
        CCE.Print("  Channels:")
        local i = 1
        while i <= #cl do
            CCE.Print("    #" .. tostring(cl[i]) .. " " .. tostring(cl[i+1]))
            i = i + 3
        end
    end
end
...
local function Init()
    ...
    C_Timer.NewTicker(300, function()
        ...
        if count > 0 and CCE.Print then
            CCE.Print("|cff4de64d" .. count .. " CCE player" .. (count == 1 and "" or "s") .. " online.|r")
        end
    end)
