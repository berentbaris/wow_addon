----------------------------------------------------------------------
-- ClassicClassesEnhanced — Doubt System
--
...
--
-- Storage: CCE_CharDB.doubtByClass = { ["ClassName"] = 54.3, … }
...
function Doubt.IsEnabled()
    ...
    if CCE_GlobalDB.doubtEnabled == nil then return true end  -- default ON
    return CCE_GlobalDB.doubtEnabled
end

--- Get or init the per-class doubt table.
local function doubtDB()
    CCE_CharDB = CCE_CharDB or {}
    CCE_CharDB.doubtByClass = CCE_CharDB.doubtByClass or {}
    return CCE_CharDB.doubtByClass
end
...
local function getFailPct()
    if not CCE.Progress or not CCE.Progress.Collect then return 0 end
...
local TUNNEL_TEXTURE_PATH = "Interface\\AddOns\\ClassicClassesEnhanced\\Textures\\tunnel_vision_%d.png"
...
local function CreateBar()
    ...
    barFrame:SetScript("OnEnter", function(self)
        ...
        if hasFailed then
            ...
            GameTooltip:AddLine(" ")
            GameTooltip:AddLine("Type /cce doubt reset to reset your doubt.", 1, 0.82, 0)
        else
...
--- Reset doubt for the current class (used by /cce doubt reset).
function Doubt.ResetDoubt()
    local key = classKey()
    if not key then
        print("|cffffcc00[CCE]|r No class selected — nothing to reset.")
        return
    end
    secureDoubt = 0
    previousDoubt = 0
