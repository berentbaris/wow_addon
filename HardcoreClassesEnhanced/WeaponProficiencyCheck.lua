----------------------------------------------------------------------
-- HardcoreClassesEnhanced — Weapon Proficiency Tracking
--
...
--   Level 60 → 295
--
-- Results are stored in HCE_CharDB.weaponProfResults so the
...
function WP.RunCheck()
    ...
    if HCE_CharDB then
        HCE_CharDB.weaponProfResults = results
    end
    return results
end
...
function WP.GetResults()
    return HCE_CharDB and HCE_CharDB.weaponProfResults or {}
end
...
function WP.PrintStatus()
    ...
    if not char or not char.weaponProficiency or #char.weaponProficiency == 0 then
        HCE.Print("Your character has no weapon proficiency requirements.")
        return
    end
    ...
    for _, wpn in ipairs(char.weaponProficiency) do
        ...
        elseif res.status == "inactive" then
            HCE.Print("  |cff595959" .. wpn .. " — " .. res.detail .. "|r")
        elseif res.status == PASS then
            HCE.Print("  |cff00ff00PASS|r " .. res.detail)
        elseif res.status == FAIL then
            HCE.Print("  |cffff3333FAIL|r " .. res.detail)
        else
            HCE.Print("  |cffa5a582?|r " .. wpn)
        end
    end
end
