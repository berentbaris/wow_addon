----------------------------------------------------------------------
-- ClassicClassesEnhanced — Progress Summary
--
...
--      (called by Panel.Refresh via CCE.Progress.BuildBar / UpdateBar)
--   2) A full chat printout via /cce progress
--
...
function Progress.Collect()
    ...
    if charSelfFound and (not CCE.SelfFoundEnabled or CCE.SelfFoundEnabled()) then
        local sfResults = CCE.SelfFoundCheck and CCE.SelfFoundCheck.GetResults and CCE.SelfFoundCheck.GetResults() or {}
    ...
    local profResults = CCE.ProfessionCheck and CCE.ProfessionCheck.GetResults and CCE.ProfessionCheck.GetResults() or {}
    ...
    local wpResults = CCE.WeaponProficiencyCheck and CCE.WeaponProficiencyCheck.GetResults and CCE.WeaponProficiencyCheck.GetResults() or {}
    ...
    -- 5) Challenges
    local chResults = CCE.ChallengeCheck and CCE.ChallengeCheck.GetResults and CCE.ChallengeCheck.GetResults() or {}
    ...
    for i, ch in ipairs(activeChallenges) do
        if ch.endLevel and playerLevel > ch.endLevel then
            add(ch.desc, "Challenges", S_PASS, "Outleveled (was lv " .. ch.level .. "-" .. ch.endLevel .. ")")
    ...
    -- 6) Quests
    local questResults = CCE.QuestCheck and CCE.QuestCheck.GetResults and CCE.QuestCheck.GetResults() or {}
    local questStatus  = CCE.QuestCheck and CCE.QuestCheck.STATUS or {}
...
function Progress.CheckRankUp()
    ...
    local oldRank = CCE_CharDB.currentRank
    CCE_CharDB.currentRank = rank
    ...
    if CCE.LevelUpSummary and CCE.LevelUpSummary.ShowRankUp then
        CCE.LevelUpSummary.ShowRankUp(rank, displayName, pct)
    end
    ...
    if CCE.Print then
        ...
        if isRankUp then
            CCE.Print("|cffffd100RANK UP!|r You are now " .. coloredRank .. " |cffffd100" .. displayName .. "|r (" .. pct .. "% complete)")
        else
            CCE.Print("|cffff4444RANK DOWN.|r You are now " .. coloredRank .. " |cffffd100" .. displayName .. "|r (" .. pct .. "% complete)")
        end
    end
end
...
function Progress.PrintStatus()
    ...
    CCE.Print("--- " .. char.name .. " (" .. char.spec .. " " .. classStr .. ") Progress ---")
    ...
    CCE.Print("|cff" .. pctCol .. pct .. "% complete|r  ("
        .. c.pass .. " met, "
        .. c.fail .. " broken, "
    ...
    for _, item in ipairs(data.items) do
        if item.category ~= lastCat then
            ...
            CCE.Print("|cff" .. COL.GOLD.hex .. item.category .. "|r")
        end
        local icon, col
        ...
        CCE.Print("  |cff" .. col .. icon .. "|r " .. item.name .. detail)
    end
end
