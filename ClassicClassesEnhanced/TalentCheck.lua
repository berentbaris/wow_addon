----------------------------------------------------------------------
-- ClassicClassesEnhanced — Talent / Spec Tracking
--
...
local function CheckTalentReqs(charName, playerLevel)
    local reqs = CCE.TalentRequirements and CCE.TalentRequirements[charName]
...
function TC.CheckAll()
    ...
    if not CCE_CharDB then return result end
...
function TC.RunCheck()
    ...
    if CCE_CharDB then
        CCE_CharDB.talentResults = result
    end
    return result
end
...
function TC.GetResults()
    return CCE_CharDB and CCE_CharDB.talentResults or {}
end
...
function TC.CheckAndWarn()
    ...
    local canChat = (not CCE.ChatWarningsEnabled) or CCE.ChatWarningsEnabled()
...
----------------------------------------------------------------------
-- Slash command handler: /cce talents
----------------------------------------------------------------------
...
function TC.PrintStatus()
    ...
    CCE.Print("Talent status (level " .. level .. ", spec: " .. char.spec .. "):")
    ...
    for i = 1, math.min(numTabs, 3) do
        ...
        CCE.Print("  " .. name .. ": " .. ptsColor .. points[i] .. "|r" .. marker)
    end
    ...
    CCE.Print("  Spec verdict: " .. tag .. " \226\128\148 " .. (result.specDetail or ""))

    -- Per-talent requirements
    if result.talentReqs and #result.talentReqs > 0 then
        CCE.Print("  Talent requirements:")
        for _, treq in ipairs(result.talentReqs) do
            ...
            CCE.Print("    " .. icon .. " " .. treq.name .. " (" .. rankStr .. ")" .. lvTag)
        end
    end
end
