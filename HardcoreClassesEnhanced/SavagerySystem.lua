----------------------------------------------------------------------
-- HardcoreClassesEnhanced — Savagery System  (Plagueshifter)
--
...
--
-- Storage: HCE_CharDB.savageryByClass = { ["Plagueshifter"] = 87.5, … }
----------------------------------------------------------------------

HCE = HCE or {}

local Savagery = {}
HCE.SavagerySystem = Savagery
...
local function savDB()
    ...
    HCE_CharDB.savageryByClass = HCE_CharDB.savageryByClass or {}
    return HCE_CharDB.savageryByClass
end
...
local function OnTick()
    ...
    if savagery <= 0 and not hasFailed then
        ...
        print("|cffff4444Type |cffffcc00/hce savagery reset|cffff4444 to reset.|r")
    end
...
local function CreateBar()
    ...
    barFrame:SetScript("OnEnter", function(self)
        ...
        if hasFailed then
            ...
            GameTooltip:AddLine(" ")
            GameTooltip:AddLine("Type /hce savagery reset to reset.", 1, 0.82, 0)
        else
            GameTooltip:AddLine(" ")
            if isShapeshifted() then
...
function Savagery.ResetSavagery()
    if not isActive() then
        print("|cffffcc00[HCE]|r Savagery only applies to " .. TARGET_CLASS .. ".")
        return
    end
    savagery  = MAX_SAVAGERY
    hasFailed = false
    saveSavagery()
    Savagery.UpdateBar()
    print("|cffffcc00[HCE]|r Savagery reset to 100 %.")
end
