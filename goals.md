...
**Current status:** Milestone 3 complete. All four tracking-equipment tasks finished. `EquipmentCheck.lua` watches PLAYER_EQUIPMENT_CHANGED and runs 30+ type-based rules (swords, maces, axes, staves, daggers, fist weapons, guns, wands, polearms, thrown, 2H weapons, shield, robe, no-chest) plus negative rules (no daggers, no wands, no guns, no robes). `CuratedItems.lua` holds confirmed item IDs for Flying Tiger Goggles, Green-tinted goggles, Gnomish goggles, Firestone, Spellstone, Wolfshead Helm, Guild Tabard, Lunar Festival Suit, Blue shirt, and Captain's hat, with `HCE.CuratedComplete` distinguishing definitive lists (hard FAIL on miss) from partial ones (UNCHECKED with "N items approved so far"). The new `ForbiddenAlert.lua` fires on NEW equipment violations: a red-accent-stripe toast stacking below the LevelAlert column, a brief red screen-edge vignette pulse (120ms in, 180ms hold, 600ms out), and an IG_QUEST_FAILED chime. Batched when multiple fire at once — first plays sound+flash, rest are silent; >3 at once collapse to a single summary toast so dirty logins don't strobe. Toggleable via `/hce forbidden`, previewable via `/hce testforbidden`. Stat threshold and weapon speed rules remain stubbed as UNCHECKED pending tooltip scanning.
...
**Task 4.1: Profession tracking** — on SKILL_LINES_CHANGED (and PLAYER_LOGIN), iterate GetNumSkillLines/GetSkillLineInfo to find the character's learned professions and compare against the required list in `char.professions`. Starting at level 5, verify the right professions are learned. Scale expected rank linearly as `5 * playerLevel` (level 20 = 100 skill, level 40 = 200, level 60 = 300). Surface a "profession not learned" or "profession falling behind" status on the requirements panel (needs a new PROFESSIONS section header) and fire a one-shot chat warning when the delta first goes negative — do NOT fire the red forbidden-item toast for this, since being behind on a profession isn't the same as equipping a forbidden item. Handle the locale-dependent profession-name problem by keeping a small English→GetSkillLineInfo-name table (or using spell IDs for the learn-profession spells, which are locale-independent) so the addon works in non-English clients later.
