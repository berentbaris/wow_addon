...
**Current status:** Milestone 6 complete — all companion, hunter pet, and mount tracking implemented. Created `CompanionCheck.lua` (~340 lines) for vanity pet tracking via `UnitName("critter")` with companion DB mapping 8 pet types. Created `HunterPetCheck.lua` (~280 lines) for hunter pet species verification via `UnitCreatureFamily("pet")` with Cat and Bear family matching. Created `MountCheck.lua` (~350 lines) for mount verification via buff scanning with spell-ID-first matching for Wolf, Skeletal horse, Ram, and Frostsaber mounts. All three modules integrate into RequirementsPanel with ✓/✗/? indicators and hover tooltips, have dedicated slash commands, and are wired into pick/reset flows. All eighteen Lua files parse cleanly.
...
**Task 7.1: Build curated item ID lists for item-source challenges** — using Wowhead Classic DB filters, compile lists of quest reward items, vendor-sold items, and profession-crafted items. These power the Renegade, Off-the-shelf, Self-made, and Partisan challenge checks.
...
- [x] **5.4** Behavioral challenges — Drifter (no hearthstone/bank), Ephemeral (no repair), Mortal pets (honour-system Revive Pet detection). Hooks: BANKFRAME_OPENED, UNIT_SPELLCAST_SENT (hearthstone + Revive Pet), MERCHANT_SHOW/MERCHANT_CLOSED + UPDATE_INVENTORY_DURABILITY (durability-snapshot repair detection)

### Milestone 6: Tracking - Companions, Pets & Mounts
- [x] **6.1** Companion (non-combat pet) tracking — detect if the correct vanity pet is summoned at the required level. Use unit checks or buff tracking for active pets
- [x] **6.2** Hunter pet tracking — for Hunter characters, verify the tamed pet species matches requirements (e.g., "Jungle cat," "Bear"). Use UnitCreatureFamily("pet") or similar API
- [x] **6.3** Mount tracking — at level 40+, verify correct mount is used. Mount detection via buff/aura scanning when mounted
...
- **5.4** Behavioral challenges — created `BehavioralCheck.lua` (~310 lines). Drifter: hooks `BANKFRAME_OPENED` for bank access and `UNIT_SPELLCAST_SENT` for hearthstone cast (spell ID 8690), with persistent violation flags in `HCE_CharDB.behavioral`. Ephemeral: hooks `MERCHANT_SHOW`/`MERCHANT_CLOSED`/`UPDATE_INVENTORY_DURABILITY` with durability-snapshot comparison — snapshots total gear durability when merchant opens, detects repair if durability increases while merchant is open or by close. Mortal pets: hooks `UNIT_SPELLCAST_SENT` for Revive Pet (spell ID 982) as honour-system warning. All violations persistent in SavedVars, clearable via `/hce reset`. Updated ChallengeCheck.lua: replaced three stubs (Drifter, Ephemeral, Mortal pets) with real delegations to BehavioralCheck. Added `BANKFRAME_OPENED`/`MERCHANT_CLOSED` to ChallengeCheck event frame. Added `/hce behavioral` slash command. Wired `BehavioralCheck.ResetTracking()` into pick/reset flows. Registered in `.toc` between `ZoneCheck.lua` and `ChallengeCheck.lua`. Milestone 5 complete. All fifteen Lua files parse cleanly via lupa `load()`. (2026-04-23)

- **6.3** Mount tracking — created `MountCheck.lua` (~350 lines). Uses `IsMounted()` + buff scanning via `UnitBuff("player", i)` to identify the active mount. Spell-ID-first matching (locale-independent) with buff-name fallback. Built `MC.MountDB` with all 4 required mount types: Wolf (6 Orc wolf spell IDs), Skeletal horse (8 Undead skeletal mount spell IDs), Ram (6 Dwarf ram spell IDs), Frostsaber (8 Night Elf saber spell IDs). Returns UNCHECKED when not mounted (no penalty for being on foot), PASS when correct mount is active, FAIL only when mounted on wrong species. Heuristic keyword fallback scans for unrecognised mount buffs ("wolf", "ram", "skeletal", "saber", etc.) to avoid false UNCHECKEDs. `UNIT_AURA` event listener with player filter triggers mount checks on mount/dismount. Clears wrong-mount warning on dismount so it re-fires per ride. Added `/hce mount` (and `/hce riding`) slash commands. Wired `MountCheck.ResetWarnings()` into pick/reset flows. Registered in `.toc` after `HunterPetCheck.lua`. (2026-04-24)

- **6.2** Hunter pet tracking — created `HunterPetCheck.lua` (~280 lines). Uses `UnitCreatureFamily("pet")` for locale-safe beast family detection. Built `HP.PetDB` mapping both hunter pet requirements: "Jungle cat" → Cat family (with creature hints for STV panthers, Winterspring frostsabers, etc.), "Bear" → Bear family (with creature hints for Dun Morogh, Hillsbrad, Felwood bears). Returns UNCHECKED when no pet is active or pet is dead (common for hunters — don't penalise dismissals), PASS when pet family matches, FAIL when wrong family. Filters by class token (HUNTER only). `UNIT_PET` event with "player" unit filter triggers checks on pet summon/dismiss/swap. Added `/hce hunterpet` (and `/hce hpet`) slash commands. Wired `HunterPetCheck.ResetWarnings()` into pick/reset flows. Registered in `.toc` after `CompanionCheck.lua`. (2026-04-24)

- **6.1** Companion (non-combat pet) tracking — created `CompanionCheck.lua` (~340 lines). Uses `UnitExists("critter")` + `UnitName("critter")` to identify active vanity pets. Built a companion database (`CC.CompanionDB`) mapping all 8 spreadsheet companion descriptions (Owl, Black cat, Parrot, Prairie dog, Cockroach, Phoenix, Mechanical, Snow rabbit) to sets of accepted creature names and summoning item IDs with provenance notes. Owl: Great Horned Owl (8500) / Hawk Owl (8501). Black cat: Black Tabby Cat (8491) / Bombay Cat (8485). Parrot: Cockatiel (8495) / Senegal (8496) / Green Wing Macaw (8492). Prairie dog: Prairie Dog Whistle (10394). Cockroach: Undercity Cockroach (10393). Phoenix: Crimson Whelpling (8499) / Dark Whelpling (10822) as thematic stand-ins (no actual Phoenix in Classic). Mechanical: Squirrel (4401) / Bombling (11825) / Lil' Smoky (11826) / Chicken (10398). Snow rabbit: Snowshoe Rabbit (8497). Bag scanning via `C_Container.GetContainerItemID` (with `GetContainerItemID` fallback) detects whether the player owns a matching pet item. Periodic 60s heartbeat via `C_Timer.NewTicker` re-checks critter state. Soft chat warnings (gold `[HCE]` prefix, not red ForbiddenAlert) fire once per session for "no companion summoned" and "wrong companion" states, with positive acknowledgement when the correct pet is summoned. Updated RequirementsPanel: companion row now shows ✓/✗/? tracking indicator with hover tooltip showing check detail. Added `COMPANION_UPDATE` (pcall-safe) to panel live-refresh events. Added `/hce companion` (and `/hce pet`, `/hce critter`) slash commands. Wired `CompanionCheck.ResetWarnings()` into pick/reset flows. Registered in `.toc` after `ItemSourceData.lua`. All sixteen Lua files parse cleanly via lupa `load()`. (2026-04-24)
