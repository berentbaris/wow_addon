----------------------------------------------------------------------
-- ClassicClassesEnhanced — Level-gated Requirement Surfacing
--
...
local function buildToast()
    ...
    -- layout sketch above, and the main hook that reads as "CCE".
    ...
    t:SetScript("OnClick", function(self, btn)
        if btn == "RightButton" and self.challengeKey then
            ...
            local desc = CCE.ChallengeDescriptions and CCE.ChallengeDescriptions[self.challengeKey]
            if desc then
...
-- @param challengeKey  optional string key into CCE.ChallengeDescriptions for
...
function Alert.Toast(section, desc, level, playSound, challengeKey)
    ...
    if challengeKey and CCE.ChallengeDescriptions and CCE.ChallengeDescriptions[challengeKey] then
    ...
    if playSound ~= false and (not CCE.AlertSoundEnabled or CCE.AlertSoundEnabled()) then
...
function Alert.Check()
    ...
    if CCE_GlobalDB and CCE_GlobalDB.alertsEnabled == false then return end
    ...
    local last = CCE_CharDB.lastLevel or current  -- first run: no backlog

    if current <= last then
        CCE_CharDB.lastLevel = current
        return
    end
...
--- Reset the level tracker.  Used by `/cce reset` and when a new
...
CCE.CheckLevelAlerts  = Alert.Check
CCE.ResyncLevelAlerts = Alert.ResyncBaseline
...
----------------------------------------------------------------------
-- Test slash command: /cce testalert
----------------------------------------------------------------------

CCE.TestAlert = function()
