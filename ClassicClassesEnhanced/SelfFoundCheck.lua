----------------------------------------------------------------------
-- ClassicClassesEnhanced — Self-Found & Self-Made Tracking
--
...
--
-- Results are stored in CCE_CharDB.selfFoundResults so the
...
-- CCE.CuratedItems.crafted_items from ItemSourceData.lua.
...
--- Call via /cce debugtooltip
function SF.DebugTooltips()
    CCE.Print("|cffffd100=== Tooltip Debug ===|r")
...
function SF.RunCheck()
    ...
    if CCE_CharDB then
        CCE_CharDB.selfFoundResults = results
    end
    return results
end
...
function SF.GetResults()
    return CCE_CharDB and CCE_CharDB.selfFoundResults or {}
end
...
function SF.PrintStatus()
    ...
    CCE.Print("Self-Found / Self-Made status:")

    local results = SF.RunCheck()

    -- Self-found buff
    local charSelfFound2
    if CCE.GetCharSelfFound then charSelfFound2 = CCE.GetCharSelfFound(char) else charSelfFound2 = char.selfFound end
    if charSelfFound2 then
        ...
        if r then
            ...
            CCE.Print("  Self-Found buff: " .. tag .. " — " .. (r.detail or ""))
        else
            CCE.Print("  Self-Found buff: |cff888888no data|r")
        end
    elseif charSelfFound2 == false then
        ...
        if r then
            ...
            CCE.Print("  Not Self-Found: " .. tag .. " — " .. (r.detail or ""))
        else
            CCE.Print("  Not Self-Found: |cff888888no data|r")
        end
    else
        CCE.Print("  Self-Found: not required for this character")
    end
    ...
    if results.selfMade then
        ...
        CCE.Print("  Self-made: " .. tag .. " — " .. (r.detail or ""))
        ...
        if r.items then
            for _, item in ipairs(r.items) do
                if item.status ~= PASS then
                    ...
                    CCE.Print("    Slot " .. item.slot .. ": " .. itemTag .. " " .. (item.detail or ""))
                end
            end
        end
    end
    ...
    if results.selfMadeGuns then
        ...
        CCE.Print("  Self-made guns: " .. tag .. " — " .. (r.detail or ""))
    end
end
