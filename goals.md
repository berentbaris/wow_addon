...
**Current status:** Milestone 4 in progress — Task 4.1 (profession tracking) complete. `ProfessionCheck.lua` watches SKILL_LINES_CHANGED, PLAYER_LOGIN, and PLAYER_LEVEL_UP. Uses locale-independent spell IDs (IsSpellKnown) for profession detection, then reads rank via GetSkillLineInfo scan with a multi-pass locale-fallback matcher. Expected rank formula: 5 × playerLevel (starts at level 5). Results stored in `HCE_CharDB.profResults`. RequirementsPanel now has a dedicated PROFESSIONS section header with ✓/✗/? indicators and hover tooltips showing rank vs expected. Chat warnings fire once per profession per state-transition ("not learned" or "falling behind") — deliberately softer than the red ForbiddenAlert toast. `/hce professions` (and `/hce prof`) prints a status table. Warning state resets on character pick/reset. All nine Lua files parse cleanly.
...
**Task 4.2: Talent/spec tracking** — starting at level 10, check talent point distribution. Use GetSpellTabInfo to count points in each tree and verify majority are in the correct spec tree. Alert if the player is falling behind on their spec. Surface results in the requirements panel and fire a one-shot chat warning (same soft style as profession warnings).
